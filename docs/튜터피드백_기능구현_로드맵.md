...
### 1.1 Feedback 테이블 생성
- [ ] 테이블 정보
  - 테이블 이름: `${StackName}-feedback`
  - Billing Mode: PAY_PER_REQUEST (온디맨드)

- [ ] 스키마 정의
  | 속성 | 타입 | 키 | 설명 |
  |------|------|-----|------|
  | `session_id` | String | **PK (HASH)** | 학생-튜터 세션 ID |
  | `timestamp` | String | **SK (RANGE)** | 메시지 발송 시각 (ISO 8601) |
  | `tutor_email` | String | - | 튜터 이메일 |
  | `student_email` | String | - | 학생 이메일 |
  | `message_text` | String | - | 피드백 메시지 텍스트 |
  | `message_type` | String | - | 메시지 타입 (text / tts) |
  | `audio_url` | String | - | TTS 오디오 S3 URL (TTS인 경우) |
  | `created_at` | String | - | 생성 시각 |

- [ ] 옵션 설정
  - TTL 설정: 30일 후 자동 삭제 (선택사항)
  - GSI 고려: `student_email-timestamp` (학생 기준 조회)

### 1.2 WebSocket Connection 테이블 (선택)
- [ ] 테이블 정보
...
- [ ] 스키마 정의
  ...
  | `user_type` | String | - | 사용자 타입 (tutor / student) |
...
### 2.1 SentencesWebSocketFunction 분석
- [ ] 기존 WebSocket 로직 이해
  - 현재 메시지 처리 방식
  - 라우팅 메커니즘
  - 에러 처리 방식
